  ...
  try {
    ...
    if (response.statusCode == 200 && response.headers.containsValue('application/json')) {
      if (json.decode(response.body)['data'] != null) {
        ...
        print("Main color: " + _setting.mainColor);
        //_setting.mainColor = "#e8b92b";
        //_setting.mainDarkColor = "#e8b92b";
...
Future<dynamic> setCurrentLocation() async {
  ...
  location.requestService().then((value) async {
    location.getLocation().then((_locationData) async {
      ...
      print("Error set current location.");
      print(e);
      whenDone.complete(_address);
    });
  });
...
Future<Address> getCurrentLocation() async {
  SharedPreferences prefs = await SharedPreferences.getInstance();
  //await prefs.clear();
  //To do: traer la ubicación desde la base, la que este marcada por default y guardarla en las preferencias 
